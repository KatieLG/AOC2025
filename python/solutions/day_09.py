...
class Day09(AOCSolution):
    ...
    def lines_intersect(self, a: Vec, b: Vec, p: Vec, q: Vec) -> bool:
        ...
        return x1 < x_intersect < x2 and (min(y1, y2) < y_intersect < max(y1, y2) or y1 == y2 == y_intersect) and (min(y3, y4) < y_intersect < max(y3, y4) or y3 == y_intersect == y4)
    ...
    def part_two(self) -> int:
        ...
        ax.plot(*a, marker="x")
        ax.plot(*b, marker="x")
        ax.add_patch(Rectangle((min(a[0], b[0]), min(a[1], b[1])), abs(b[0]-a[0]), abs(b[1]-a[1]), fill=True, color="red", zorder=10, alpha=.5))
        plt.show()
        print(largest_rect)
